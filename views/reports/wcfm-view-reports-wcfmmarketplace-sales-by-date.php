  ...
  <div class="wcfm-page-headig">
		...
		<span class="wcfm-page-heading-text">
		  <?php if( $sales_by_vendor_mode ) { ?>
		  	<?php if ( 'custom' === $current_range && isset( $_GET['start_date'], $_GET['end_date'] ) ) : ?>
				<?php _e('Sales BY Vendor', 'wc-frontend-manager'); ?> - <?php echo esc_html( sprintf( _x( 'From %s to %s', 'start date and end date', 'wc-frontend-manager' ), wc_clean( $_GET['start_date'] ), wc_clean( $_GET['end_date'] ) ) ); ?><span></span>
				<?php else : ?>
					<?php _e('Sales BY Vendor', 'wc-frontend-manager'); ?> - <?php echo esc_html( $ranges[ $current_range ] ); ?><span></span>
				<?php endif; ?>
		  <?php } else { ?>
				<?php if ( 'custom' === $current_range && isset( $_GET['start_date'], $_GET['end_date'] ) ) : ?>
	...
	<div class="wcfm-collapse-content">
	  ...
	  <?php if( $sales_by_vendor_mode ) { ?>
	  	<div class="wcfm-container wcfm-top-element-container">
	  	  <?php
	  	  $vendor_store = $WCFM->wcfm_vendor_support->wcfm_get_vendor_store_name_by_vendor( $WCFMmp->vendor_id );
	  	  ...
	  	  ?>
				...
				<?php
				echo '<a class="add_new_wcfm_ele_dashboard text_tip" href="'.get_wcfm_vendors_manage_url($WCFMmp->vendor_id).'" data-tip="' . __( 'Manage Vendor', 'wc-frontend-manager' ) . '"><span class="wcfmfa fa-user-alt"></span></a>';
				?>
				<div class="wcfm-clearfix"></div>
			</div>
